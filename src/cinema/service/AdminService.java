...
public final class AdminService {
    ...
    public long purchaseTicketForClient(long clientId, int year, int month, int day, long screeningId, int seatNumber) throws CinemaException, IOException, SQLException {
        ...
        if (!(0 < seatNumber && seatNumber <= auditorium.getNumber_of_seats())) {
            throw new CinemaException("The seat id " + seatNumber + " is negative or more than " + auditorium.getNumber_of_seats());
        }

        // check if that seat is taken;
        for (TicketPurchase ticketPurchase : this.getterService.getAllTicketPurchase()) {
            if (ticketPurchase.getScreeningId() == screeningId && ticketPurchase.getSeatNumber() == seatNumber) {
                throw new CinemaException("The seat id " + seatNumber + " is already taken for that screening10");
            }
        }
